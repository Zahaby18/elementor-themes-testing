<?php
/**
 * Agenzy Kit — Demo Posts Generator
 *
 * Generate 10 demo blog posts with featured images so the blog grid
 * (Jeg Elementor Kit `jkit_post_block`) has content to show.
 *
 * Run via WP-CLI:
 *   wp eval-file demo-posts.php
 *
 * (Images are downloaded from the kit's jsDelivr CDN into your Media Library.)
 */
if ( ! defined( 'ABSPATH' ) ) {
	echo "Run via WP-CLI: wp eval-file demo-posts.php\n";
	exit;
}
...
foreach ( $posts as $p ) {
	...
	if ( is_wp_error( $attachment_id ) ) {
		echo "POST {$post_id}: {$p['title']} (no featured image: " . $attachment_id->get_error_message() . ")\n";
	} else {
		set_post_thumbnail( $post_id, $attachment_id );
		echo "POST {$post_id}: {$p['title']} (featured image set)\n";
	}
	$created++;
}
